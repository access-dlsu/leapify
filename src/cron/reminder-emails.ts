...
export async function reminderEmails(env: LeapifyBindings): Promise<void> {
  if (!env.EMAIL_QUEUE) {
    console.warn('[reminder-emails] EMAIL_QUEUE binding not configured, skipping.')
    return
  }
  ...
  for (const event of events24h) {
    ...
    if (hoursUntil <= 25 && hoursUntil >= 23) {
      await env.EMAIL_QUEUE.send({ type: 'send_reminder_email', payload: { eventId: event.id, hoursBeforeEvent: 24 } })
    }
  }
  ...
  for (const event of events1h) {
    ...
    if (minutesUntil <= 65 && minutesUntil >= 55) {
      await env.EMAIL_QUEUE.send({ type: 'send_reminder_email', payload: { eventId: event.id, hoursBeforeEvent: 1 } })
    }
  }
